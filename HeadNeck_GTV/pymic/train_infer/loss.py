...
def get_ce_loss(output_tensor,target_tensor):

    target_tensor = target_tensor.long().squeeze()
    return F.cross_entropy(output_tensor, target_tensor)
    # target_tensor = target_tensor.squeeze()
    # print(target_tensor.shape)
    # return F.binary_cross_entropy_with_logits(output_tensor,target_tensor)
...
def soft_dice_loss(predict, soft_y, softmax = True):
    # print(predict.shape,soft_y.shape,'predict-soft-y before')
    if(softmax):
...
class SegmentationLossCalculator():
    ...
    def get_loss(self, predict, softy, softmax = True):
        dice_loss = segmentation_loss_dict[self.loss_name](predict,softy, softmax)
        return dice_loss

...
class RandomFlipImage(object):
    ...
    Args:
        ...
        flip_width (bool) : random flip along width axis or not
    """
    def __init__(self, flip_depth=False, flip_height=False, flip_width=False, inverse=False):
        self.flip_depth  = flip_depth
        self.flip_height = flip_height
        self.flip_width  = flip_width
        self.inverse = inverse
    ...
    def __call__(self, image,label=None):
        ...
        if(len(flip_axis) > 0):
            # use .copy() to avoid negative strides of numpy array
            # current pytorch does not support negative strides
            image = np.flip(image, flip_axis).copy()
            label = np.flip(label, flip_axis).copy()
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if('label' in sample):
            ...
            crop_max[0] = label.shape[0]
            label = crop_ND_volume_with_bounding_box(label, crop_min, crop_max)
        # print(input_shape,crop_min,crop_max)
        if need_pad:
            input_shape = image.shape
            image_temp = np.zeros([1]+list(self.output_size),dtype=np.float32)
            label_temp = np.zeros([1]+list(self.output_size),dtype=np.int32)
            image_temp[:input_shape[0],:input_shape[1],:input_shape[2],:input_shape[3]] = image[:,:,:,:]
            label_temp[:input_shape[0],:input_shape[1],:input_shape[2],:input_shape[3]] = label[:,:,:,:]
            image = image_temp
            label = label_temp
        sample['label'] = label
        sample['image'] = image
...
class ExtractPatches(object):
    def __init__(self,patch_shape):
        self.patch_depth = patch_shape[0]
        self.patch_xy = patch_shape[1]
    
    def __call__(self,sample):
        extracted_patches = []
        if 'label' in sample:
            label = sample['label'].squeeze()
            extracted_patches_label = []
        image_3D = sample['image'].squeeze()
        input_shape = image_3D.shape
        depth_length = input_shape[0]//self.patch_depth
        spatial_length = input_shape[1]//self.patch_xy

        for depth_i in range(depth_length):
            for space_i in range(spatial_length):
                for space_j in range(spatial_length):
                    patch_3D = image_3D[
                        depth_i*self.patch_depth:(depth_i+1)*self.patch_depth,
                        space_i*self.patch_xy:(space_i+1)*self.patch_xy,
                        space_j*self.patch_xy:(space_j+1)*self.patch_xy
                        ]
                    extracted_patches.append(patch_3D)
                    if 'label' in sample:
                        patch_3D_label = label[
                            depth_i*self.patch_depth:(depth_i+1)*self.patch_depth,
                            space_i*self.patch_xy:(space_i+1)*self.patch_xy,
                            space_j*self.patch_xy:(space_j+1)*self.patch_xy,
                            ]
                        extracted_patches_label.append(patch_3D_label)
                    # print(patch_3D.shape,patch_3D_label.shape)

        extracted_patches = np.stack(extracted_patches)
        extracted_patches_label = np.stack(extracted_patches_label)
        sample['label'] = extracted_patches_label
        sample['image'] = extracted_patches
        return sample
...
def get_transform(name, params):
    ...
    elif (name == 'ExtractPatches'):
        return ExtractPatches(params['RandomCrop_output_size'.lower()] )

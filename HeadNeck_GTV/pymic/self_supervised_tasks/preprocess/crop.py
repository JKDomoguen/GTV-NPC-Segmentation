...
def crop_3d(image, is_training, crop_size):
    ...
    if len(image.shape) == 4:
        image = image.squeeze()
        crop_size = (crop_size[1],crop_size[2],crop_size[3])

    h, w, d = crop_size[0], crop_size[1], crop_size[2]
    d_old, h_old, w_old = image.shape[0], image.shape[1], image.shape[2]

    # print(d,d_old)
    # print(h,h_old)
    # print(w,w_old)
    # print(d_old-d)
    if (d_old-d) < 0 or (h_old-h)<0 or (w_old-w)<0:
        return image
...
def do_crop_3d(image, x, y, z, h, w, d):
    ...
    return image[z:z + d, x:x + h, y:y + w]
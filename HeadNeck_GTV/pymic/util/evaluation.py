...
def sensitivity(s, g, resize = False):
    assert(len(s.shape)== len(g.shape))
    if(resize):
        size_match = True
        for i in range(len(s.shape)):
    ...
    s1 = s.sum()
    s2 = g.sum()
    sensitivity_val = (s0 + 1e-5)/(s2 + 1e-5)
    return sensitivity_val

def specificity(s, g, resize = False):
    assert(len(s.shape)== len(g.shape))
    if(resize):
        size_match = True
        for i in range(len(s.shape)):
        ...
        if(size_match is False):
            s = resize_ND_volume_to_given_shape(s, g.shape, order = 0)
    s_inverted = np.where(s==1,0,1)
    g_inverted = np.where(g==1,0,1)
    prod = np.multiply(s_inverted, g_inverted)
    s0 = prod.sum()
    s1_inverted = s_inverted.sum()
    s2 = g.sum()
    specificity_val = (s0 + 1e-5)/(s1_inverted + 1e-5)
    return specificity_val


def ppv_func(s, g, resize = False):
    assert(len(s.shape)== len(g.shape))
    if(resize):
        size_match = True
        for i in range(len(s.shape)):

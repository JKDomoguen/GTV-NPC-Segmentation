...
class TrainInferAgent():
    ...
    def __train(self):
        self.train_txt = open('train_out_1.txt','a')
        ...
        for it in range(iter_start, iter_max):
            ...
            print("Epoch-->{}, Iteration-->{}".format(epoch,it),file=self.train_txt)
            if it %loader_length == 0 and it>0:
                epoch += 1
        ...
        # for epoch in range(1,total_epoch):
            ...
            if (it % iter_valid == iter_valid - 1):
                print("Begin validation at Epoch-->{} and Iteration-->{}".format(epoch,it))
                print("Begin validation at Epoch-->{} and Iteration-->{}".format(epoch,it),file=self.train_txt)
                ...
                print('valid cls dice', valid_cls_dice.shape, valid_cls_dice,file=self.train_txt)
                ...
                print("{0:} it {1:}, loss {2:.4f}, {3:.4f}".format(
                    str(datetime.now())[:-7], it + 1, train_avg_loss, valid_avg_loss),file=self.train_txt)
...
#june2021-rad-seg-2 -- batch-size 8
